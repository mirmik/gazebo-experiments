...
namespace gazebo
{
	...
	class RemoteControlled : public ModelPlugin
	{
		...
		void Load(physics::ModelPtr _parent, sdf::ElementPtr /*_sdf*/) override
		{
			...
			{
				auto& joint = **it;
				nos::fprintln("\t{} type: {}", joint.GetName(), joint.GetType());
			}
		...
		void publish(const std::string theme, const ignition::math::Pose3d & pose) 
		{
			auto lin = pose.Pos();
			auto ang = pose.Rot();
			auto msg = nos::format("{{'lin':[{},{},{}],'ang':[{},{},{},{}]}",
				lin.X(), lin.Y(), lin.Z(), ang.X(), ang.Y(), ang.Z(), ang.W());
		...
		void OnUpdate()
		{
			...
			publish(remotectr_theme + _parent->GetName() + "/world_pose", pose);
				 ...
				 ++it) 
			{
				...
				publish(
					remotectr_theme + _parent->GetName() 
					+ "/" + link.GetName() 
					+ "/world_pose", pose);				
			}
				 ...
				 ++it) 
			{
				...
				for (int i = 0; i < dof; ++i) 
				{
					double position = joint.GetPosition(i);
